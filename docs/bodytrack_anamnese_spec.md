...
- `objetivo_principal` — `multi` — hipertrofia / composição corporal / performance esportiva / saúde geral / dor e reabilitação / correção postural / condicionamento cardiorrespiratório / flexibilidade-mobilidade / disposição e energia *(as duas últimas desde a spec 1.1; cobrem "objetivos secundários" sem campo novo)*
- `objetivo_motivo` — `text` *(1.1)* — por que o objetivo importa hoje (âncora motivacional/adesão)
- `objetivo_6meses` — `text` *(1.1)* — onde quer estar em 6 meses
...
### B1b. Logística e preferências de treino *(desde a spec 1.1)*

Alimenta diretamente a montagem do treino (frequência, tempo, equipamento) e a
adesão (preferências e vetos).

- `treino_freq_semana` — `enum` — 1..6x por semana / ainda não sei
- `treino_tempo_sessao` — `enum` — até 30 / 30-45 / 45-60 / 60-90 / >90 min
- `treino_local` — `enum` — academia / casa (home gym) / academia do condomínio / parque-ao ar livre
- `treino_equipamentos` — `text` — exibido quando o local não é academia (progressive disclosure)
- `pref_gosta` / `pref_nao_gosta` — `text` — exercícios/treinos que mais e menos gosta
- `pref_veto` — `text` — exercício que NÃO quer fazer por qualquer motivo (acionável no montador de treino)
- `perfil_sessao` — `enum` — curta e intensa / volumosa e cadenciada
...
- `lesoes_diagnosticadas` — `multi` — LCA / menisco / manguito rotador / luxação recidivante / fratura recente (<1 ano) / hérnia de disco / tendinite-tendinopatia crônica / estiramento grave (grau II-III) / outra
- `lesoes_estado_atual` — `text` — condicional (alguma marcada): operado, liberado pelo médico, instabilidade...

As lesões de região inequívoca (LCA, menisco, manguito, hérnia de disco)
alimentam os sinais de revisão de exercício (`contraindications.ts`); as demais
não têm região implícita e ficam pro julgamento do profissional.
