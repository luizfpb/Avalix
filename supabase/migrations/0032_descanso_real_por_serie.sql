-- Avalix — migration 0032: descanso realizado entre as séries.
-- Depende de 0031. Campo opcional, em segundos; NULL significa não informado
-- e zero significa que não houve descanso. A prescrição continua independente.
-- O teto de uma hora acomoda pausas reais maiores que o descanso prescrito.
-- Aplicação manual pelo usuário, antes de publicar o frontend correspondente.

begin;

alter table public.workout_log_sets
  add column rest_seconds integer,
  add constraint workout_log_sets_rest_seconds_check
    check (rest_seconds is null or rest_seconds between 0 and 3600);

comment on column public.workout_log_sets.rest_seconds is
  'Descanso realizado após a série, em segundos; NULL = não informado, 0 = sem descanso.';

-- O profissional mantém a escrita atômica e a RLS (security invoker).
-- p_sets aceita rest_seconds opcional; clientes e registros antigos usam NULL.
...
revoke execute on function public.create_workout_log(uuid, jsonb, text, int, date, text)
  from anon, public;
grant execute on function public.create_workout_log(uuid, jsonb, text, int, date, text)
  to authenticated;

-- Atualiza somente a implementação interna da 0027. A fachada pública da
-- 0028 permanece com advisory lock, revisão monotônica e data imutável.
-- Os limites, validações de escopo, token, atomicidade e idempotência continuam.
...
-- A referência da última série e os dois históricos devolvem o valor realizado.
-- Nenhuma leitura usa o descanso prescrito como substituto para dado ausente.
...
as $$
  ...
  from (
    select chosen.exercise_id, chosen.performed_at,
           chosen.weight_kg, chosen.reps, chosen.rir, chosen.rest_seconds
      from (
        select s.exercise_id, l.performed_at, s.weight_kg, s.reps, s.rir, s.rest_seconds,
               row_number() over (
...
revoke execute on function public.get_workout_history_for_link(text, int, date)
  from public;
grant execute on function public.get_workout_history_for_link(text, int, date)
