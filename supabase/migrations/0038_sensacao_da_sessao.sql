-- 0038 — sensação da sessão registrada pelo aluno.
-- Aplicar depois da 0037. Não altera RLS, policies nem o que já está gravado;
-- acrescenta uma coluna opcional e a repassa nas RPCs do link do aluno.
--
-- A tela do aluno já pergunta "como foi o treino?" em texto livre, e quase
-- ninguém digita no meio da academia. Três carinhas custam um toque e dão ao
-- profissional o sinal que antecede lesão e abandono — mas só valem como dado
-- se forem uma coluna, e não um prefixo enfiado na observação.
--
-- 1 = foi difícil / pesado, 2 = normal, 3 = foi bem / leve. Nulo continua
-- sendo a resposta mais comum: o campo é opcional e ninguém é obrigado a
-- responder para concluir o treino.
--
-- As duas funções de envio são recriadas inteiras (é o padrão das 0032/0033:
-- a versão vigente mora num arquivo só). Como a assinatura ganha um argumento,
-- elas são derrubadas antes — CREATE OR REPLACE criaria uma SOBRECARGA, e o
...
do $$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'workout_logs_feel_chk'
  ) then
    alter table public.workout_logs
      add constraint workout_logs_feel_chk check (feel is null or feel between 1 and 3);
  end if;
end $$;

comment on column public.workout_logs.feel is
  'Sensação relatada pelo aluno ao concluir: 1 difícil, 2 normal, 3 bem. Nulo = não respondeu.';
...
begin
  ...
  if v_log.id is null then
    insert into public.workout_logs
      (plan_id, day_label, week_number, performed_at, notes, source, client_ref, feel)
    values
...
-- Helper interno: nenhum cliente chama direto (o wrapper da concorrência é a
-- porta de entrada). Reemitido porque o DROP acima levou os grants junto.
...
revoke execute on function public.submit_workout_session(
  text, uuid, jsonb, text, int, date, text, uuid, int, int) from public;
grant execute on function public.submit_workout_session(
  text, uuid, jsonb, text, int, date, text, uuid, int, int) to anon, authenticated;

-- A correção de uma sessão enviada devolve a sessão inteira, e a tela troca o
-- item da lista por esse retorno: sem a sensação aqui, a carinha sumiria da
-- tela depois de uma correção mesmo continuando gravada. A correção não altera
-- a sensação — quem respondeu foi o aluno, na hora.
...
-- Histórico do aluno passa a devolver a sensação junto da sessão. Mesma
-- assinatura, então CREATE OR REPLACE preserva os grants da 0028/0032/0033.
...
begin
  ...
  with page as materialized (
    select l.id, l.plan_id, l.performed_at, l.day_label, l.week_number, l.notes,
           l.source, l.feel, l.created_at, l.updated_at, p.name as plan_name
      from public.workout_logs l
